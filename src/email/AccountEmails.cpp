/**
 * @file AccountEmails.cpp
 * @brief Bodies for src/email/AccountEmails.hpp — compiled once into
 *        app_core. Holds the jobs/Jobs.hpp include that used to sit in the
 *        header, so the email→jobs edge stays out of the header plane
 *        (Mailer.cpp set the pattern). The routing and best-effort
...
json user_ctx(const Domain::User& u) {
    return json{
        {"email", u.email},
 ...
 */
void render_and_send(const std::string& template_name,
 ...
 */
void dispatch(const std::string& kind, const Domain::User& user, const std::string& new_email = "") {
